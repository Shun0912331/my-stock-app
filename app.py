    ...
    try:
        ...
        for index, row in df.iterrows():
            if pd.notna(row['代號']):
                ...
                category = str(row.get('分類', '本人')).strip()
    ...
    if MY_PORTFOLIO:
        ...
        for i, info in enumerate(MY_PORTFOLIO):
            ...
            if current_price is not None and prev_price is not None:
                ...
                buy_fee = max(20, stock_cost_raw * 0.001425 * 0.6)
                sell_fee = max(20, stock_value_raw * 0.001425 * 0.6)
...
with tab3:
    ...
    if not df_market.empty:
        ...
        if not df_market[df_market["代號"] == "^TWOII"].empty:
            ...
            idx_cols[1].metric(label="📈 櫃買指數", value=f"{twoii['最新報價']:,.2f}", delta=f"{twoii['漲跌點數']:.2f} ({twoii['漲跌幅 (%)']}%)", delta_color="inverse")
